...
namespace Trace.Controllers
{
    ...
    public class BillingConfigurationController : ControllerBase
    {
        ...
        [HttpPost("/AddOperationalArea")]
        public async Task<IActionResult> AddOperationalArea([FromBody] OperationArea area) 
        {
            try
            {
                await _billingConfigurationRepository.AddOperationArea(area);
        ...
        public async Task<IActionResult> GetOperationAreas() 
        {
            try
            {
                var areas = await _billingConfigurationRepository.GetOperationAreas();
                return Ok(areas);
            }catch(ArgumentException ex) 
            {
                return BadRequest($"{ex.Message}");
        ...
        public async Task<IActionResult> AddSubTerritory([FromBody] SubTerritory subTerritory) 
        {
            try
            {
                ...
                return Ok("Sub Territory Added Successfully");
            }
            catch (Exception)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, "An error occurred while processing your request.");
